...
change_face_list = ["beard1.jfif", "beard1.jfif"]
            ...
            for face_landmarks in results.multi_face_landmarks:
                ...
                if len(landmark_target_ocv) > 0:
                    ...
                    if process == True:
                        ...
                        # img2_gray = img2_gray-img2_new_face;
                        # img2_gray = img2_gray-(cv2.cvtColor(img2_new_face,cv2.COLOR_BGR2GRAY))
                        img2_face_mask = np.zeros_like(img2_gray)
                        img2_head_mask = cv2.fillConvexPoly(img2_face_mask, convexhull2, 255)
                        img2_face_mask = cv2.bitwise_not(img2_head_mask)
                        cv2.imshow("mask1", img2_face_mask)

                        img2_head_noface = cv2.bitwise_and(seam_clone, seam_clone, mask=img2_face_mask)
                        cv2.imshow('headno', img2_new_face)
                        img_no_mask = np.zeros_like(img2_new_face);
                        img2_head_mask_no = cv2.fillConvexPoly(img_no_mask, convexhull2, 255)
                        facecoordbeard = [];
                        for n in range(len(tmp.TRIANGULATION1)):
                            facecoordbeard.append(landmark_target_ocv[tmp.TRIANGULATION1[n]])

                        img2_face_mask_no = cv2.bitwise_not(img2_head_mask_no)
                        # facecoordbeard = np.zeros_like(facecoordbeard)


                        # img_noface_mask = np.zeros_like(img2_new_face);
                        result = cv2.add(img2_head_noface, img2_new_face)
                        img_no_mask = cv2.fillPoly(img2_new_face, np.int32([np.array(facecoordbeard)]), (255, 255, 255))
                        cv2.imshow('maskss', img_no_mask);
                        (x, y, w, h) = cv2.boundingRect(convexhull2)
                        center_face2 = (int((x + x + w) / 2), int((y + y + h) / 2)+32)

                        seamlessclone = cv2.seamlessClone(result, seam_clone,
                                                          img_no_mask, center_face2, cv2.MIXED_CLONE)


        cv2.imshow("input", base_face_handler["img"])
        cv2.imshow('MediaPipe FaceMesh', out_image)
        cv2.imshow("seams", seam_clone)
